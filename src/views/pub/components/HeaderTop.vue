<template>
  <div class="header-wrap">
    <div class="header-menu">
      <button type="button" class="menuMobile" @click="deviceMenuToggle">
        <span class="blind">mobileMenu</span>
      </button>
      <h1 class="skSignet-logo">
        <img src="@/assets/images/skSignet-logo.png" alt="skSignet" />
      </h1>
      <button type="button" class="btn-allMenu" @click="allMenuToggle">
        <span>
          메뉴
    ...
    <div class="joinInfo-wrap">
      <div class="date">
        {{ year }}년 {{ month }}월 {{ today }}일 ({{ day }}) {{ hours }} :
        {{ minutes }}
      </div>
      <div class="alarmWrap">
        <button type="button" class="superUser" @click="superUserToggle">
          <span>2</span>
        </button>
        <button type="button" class="notice" @click="noticeToggle">
          <span>2</span>
        </button>
        <button type="button" class="alarm" @click="errorToggle">
          <span>2</span>
        </button>
      </div>
    ...
    <!-- Super User -->
    <super-user-list :superUserIsActive="superUserIsActive" />
    <!-- 공지사항 리스트 -->
    <notice-list :noticeIsActive="noticeActive" />
    <!-- 알림 리스트 -->
    <error-list :errorIsActive="errorActive" />
    <!-- 전체메뉴 -->
    <transition name="slide">
      <div v-if="allMenuActive" class="header-allMenu">
        <all-menu />
      </div>
    </transition>
  </div>
...
export default {
  ...
  methods: {
    ...
    // })
    deviceMenuToggle() {
      this.$store.commit("settings/DEVICEMENU_TOGGLE");
    },
  },
};
</script>
